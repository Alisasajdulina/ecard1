...
interface CardDao {
    ...
    @Query("UPDATE cards SET viewCount = viewCount + 1, lastViewed = :timestamp WHERE id = :id AND userId = :userId")
    suspend fun incrementViewCount(id: Long, userId: Long, timestamp: Long)
    ...
    @Query("DELETE FROM cards WHERE id = :id AND userId = :userId")
